...
export class TradeInfoAnalyzer {
  ...
  analyzeFlow() {
    ...
    let accHasMinAskPrice = this.tradeAccounts
      .filter(
        (item) =>
          item.tradingPlatform.name !== accHasMaxBidPrice.tradingPlatform.name
      )
      .reduce((prev, current) => {
  ...
  analyzeFixedMode(quantity, plusPointToWin) {
    let deltaBidAsk =
      this.sellAccount.currentBidPrice - this.buyAccount.currentAskPrice;
    let deltaBidBid =
      this.sellAccount.currentBidPrice - this.buyAccount.currentBidPrice;
